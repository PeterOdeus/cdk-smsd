...
public class SMSDNormalizer {
    ...
    public static IMolecule makeDeepCopy(IAtomContainer container) {
        ...
        IMolecule newAtomContainer = container.getBuilder().
            newMolecule();
...
//      Deep copy of the LonePairs
        for (int f = 0; f < container.getLonePairCount(); f++) {
            ...
            if (container.getAtom(f).getSymbol().equalsIgnoreCase("R")) {
                lonePairs[f] = container.getBuilder().newLonePair(container.getAtom(f));
            }
            newAtomContainer.addLonePair(lonePairs[f]);
        }

        for (int f = 0; f < container.getSingleElectronCount(); f++) {
            singleElectrons[f] = container.getBuilder().newSingleElectron(container.getAtom(f));
...
//		Smiles=Smiles.replace("#N=O","#[N+][O-]");
    ...
    public static boolean fixNitroGroups(IMolecule mol) {
...
//                boolean nitro = false;
                ...
                if (atom.getSymbol().equals("N")) {
                    List<IAtom> connectedAtom = mol.getConnectedAtomsList(atom);
                    ...
                    if (connectedAtom.size() == 3) {
                        ...
                        int count = 0;
                        for (int j = 0; j <= 2; j++) {
                            if (connectedAtom.get(j).getSymbol().equals("O"))
                                count++;
                        }
                        ...
                        if (count > 1) {
                            ...
                            for (int j = 0; j <= 2; j++) {
                                ...
                                if ((caj.getSymbol().equals("O")) &&
                                    (mol.getConnectedAtomsCount(caj) == 1)) {
                            ...
                            IBond.Order order1 =
                                mol.getBond(atom, cao[0]).getOrder();
                            IBond.Order order2 =
                                mol.getBond(atom, cao[1]).getOrder();

                            //if (totalobonds==4) { // need to fix (FIXME)
                            if (order1 == IBond.Order.SINGLE &&
                                order2 == IBond.Order.DOUBLE) {
                                atom.setFormalCharge(1);
                                // pick first O arbitrarily
                                cao[0].setFormalCharge(-1);
                                mol.getBond(atom, cao[0]).setOrder(
                                    IBond.Order.SINGLE
                                );
                                changed = true;
                            }
...
//						totalobonds+=mol.getBond(atom,cao[1]).getOrder();
                            ...
                            if ((order1 == IBond.Order.SINGLE &&
                                ...
                                mol.getBond(atom, cao[1]).setOrder(
                                    IBond.Order.DOUBLE
                                );
                                changed = true;
                            }
            ...
            if (atom.getSymbol().equals("S")) {
                ...
                if (bondOrderSum > 1) {
                    for (int j = 0; j < connectedAtoms.size(); j++) {
                        IAtom conAtom = (IAtom) connectedAtoms.get(j);
                        if (conAtom.getSymbol().equals("H")) {
                            mol.removeAtomAndConnectedElectronContainers(
                                conAtom
                            );
                        }
                    }
...
//            Recompute hydrogen counts of neighbours of removed Hydrogens.
    ...
    private static IAtom[] copyAtoms(IAtomContainer container, IMolecule newAtomContainer) {
        ...
        for (int f = 0; f < container.getAtomCount(); f++) {
            ...
            if (container.getAtom(f) instanceof IPseudoAtom) {
                atoms[f] = container.getBuilder().newPseudoAtom(container.getAtom(f));
            } else {
                atoms[f] = container.getBuilder().newAtom(container.getAtom(f));
            }
    ...
    private static void copyBonds(IAtom[] atoms, IAtomContainer container, IMolecule newAtomContainer) {
        ...
        for (int f = 0; f < container.getBondCount(); f++) {
            bonds[f] = container.getBuilder().newBond();
            int IndexI = 999;
            ...
            IBond.Stereo stereo = container.getBond(f).getStereo();
            bonds[f] = container.getBuilder().newBond(atom1, atom2, order, stereo);
